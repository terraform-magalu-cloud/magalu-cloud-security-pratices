...
__rego_metadata__ := {
    "id": "MGC.NET.001", "title": "Public SSH access is not allowed",
    "description": "Detects security group ingress rules that allow unrestricted access to SSH (port 22).",
...
deny contains msg if {
    ...
    msg := sprintf("Resource '%s' exposes SSH (port 22) to the public internet.", [resource.address])
}